...
const ProfileView: React.FC<ProfileViewProps> = ({ progress, bookmarks, user, authLoading, onLogin, onLogout, onSelectBookmark, onResetData, onGoToAdmin }) => {
  const level = Math.floor(progress.xp / 100) + 1;
  const nextLevelXp = level * 100;
  const currentLevelXp = progress.xp % 100;
  const levelProgress = (currentLevelXp / 100) * 100;
  const userName = user?.fullName || user?.primaryEmailAddress?.emailAddress || 'Kawan Mengaji';
  const userAvatar = user?.imageUrl;

  return (
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      {/* Auth Card */}
      <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center overflow-hidden">
            {userAvatar ? <img src={userAvatar} alt={userName} className="w-full h-full object-cover" /> : <span className="text-lg">👤</span>}
          </div>
          <div>
            <p className="text-xs text-slate-400 dark:text-slate-500">Akun</p>
            <p className="text-sm font-bold text-slate-800 dark:text-white">{user ? userName : 'Belum login'}</p>
          </div>
        </div>
        {user ? (
          <button onClick={onLogout} className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-black uppercase tracking-widest rounded-xl">
            Logout
          </button>
        ) : (
          <button onClick={onLogin} disabled={authLoading} className="px-4 py-2 bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl disabled:opacity-60">
            Login Google
          </button>
        )}
      </div>
      ...
      <div className="flex flex-col items-center pt-8">
        <div className="relative group">
          ...
          <div className="absolute -bottom-1 -right-1 w-8 h-8 rounded-full bg-emerald-500 border-4 border-white dark:border-slate-800 flex items-center justify-center text-white text-[10px] font-black">Lvl {level}</div>
        </div>
        <h2 className="mt-4 text-2xl font-black text-slate-800 dark:text-white tracking-tight">{userName}</h2>
      ...
      <div className="bg-emerald-900 dark:bg-emerald-950 p-6 rounded-[2.5rem] text-white shadow-xl shadow-emerald-900/20">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="text-xs font-bold bg-emerald-800/50 px-3 py-1 rounded-full">
            {progress.xp} / {nextLevelXp} XP
          </span>
        </div>
        <div className="h-4 bg-emerald-800/40 rounded-full overflow-hidden mb-2">
          <div className="h-full bg-emerald-400 rounded-full transition-all duration-1000" style={{ width: `${levelProgress}%` }}></div>
        </div>
        <p className="text-emerald-200 text-[10px] text-center">
          {nextLevelXp - (progress.xp % 100)} XP lagi untuk Level {level + 1}
        </p>
      </div>
